...
import { searchUsersToInvite, sendGroupInvite } from './actions';
...
export function InviteSearch({
  ...
  const handleChange = (value: string) => {
    ...
    debounceRef.current = setTimeout(() => {
      startSearch(async () => {
        setResults(await searchUsersToInvite(groupId, value, existingMemberIds));
      });
    }, 300);
  };

  const handleInvite = (invitedUserId: string) => {
    setSentIds((prev) => new Set(prev).add(invitedUserId));
    startSearch(async () => {
      const { error } = await sendGroupInvite(groupId, invitedUserId);
      if (error) {
        window.alert(error);
        setSentIds((prev) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {query.trim().length < 2 ? (
        ...
        <p className="text-center text-sm text-text-secondary">No players found</p>
      ) : (
