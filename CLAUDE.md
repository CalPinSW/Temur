...
- **Games** — create a game (friends-invite mode or group mode), set kickoff time / "visible from" time / team names / players-per-team; players sign up and get waitlisted once capacity (`players_per_team * 2`) is exceeded; withdraw; admins can add/remove guest "ringers", invite more friends, and notify players of team assignments. The creator (specifically — not any group admin) can soft-delete a game (`deleted_at`, via the `delete_game` RPC); once deleted it disappears from every read, for everyone, with no recovery UI. On the games list screen, a game not yet visible to everyone (before `visible_at`) still shows to its admin/creator as an early preview, greyed out and non-clickable (`getGameVisibilityStatus` in `packages/shared`) — distinct from being fully open, which only happens once `visible_at` passes. The game detail page itself has no equivalent visibility gating (`can_view_game`'s RLS check doesn't consider `visible_at` at all — only group membership, creator, or an explicit invite), so web's post-creation redirect checks `visible_at` before deciding where to land: straight into the new game if it's already visible, back to the games list otherwise, rather than walking the creator into a page they can't yet reach through the app's own UI. Mobile's create-game flow always returns to the list regardless, which sidesteps the question entirely.
