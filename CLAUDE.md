...
- **Account deletion** — a signed-in user can permanently delete their own account from Profile → "Delete Account", behind a type-your-username-or-email confirmation on both apps (no simple confirm dialog, unlike other destructive actions here — this one's irreversible at the auth level). The `delete-account` edge function (service-role client, JWT-authenticated caller via `getAuthedUserId`) soft-deletes any group where the caller is the *sole* member (same mechanism as `delete_group`, run first since the evidence of "who else is in this group" disappears the instant the account is gone), then calls `supabase.auth.admin.deleteUser()`. That cascades through `profiles.id ON DELETE CASCADE` to remove everything the user owns — friendships, game signups, ratings given *and* received (a rating hangs off the rated player's `player_games` row, which cascades away with them), saved ringer names — and fires the existing avatar-cleanup storage trigger; games/groups/ringer slots they created for others persist with the creator field nulled out (`ON DELETE SET NULL`, matching `games.created_by`). Bug reports survive anonymized (`user_id` set NULL) rather than being purged, matching the existing bug-report retention policy. Signing up again later, even with the same email, always creates an unrelated new account — Supabase issues a fresh UUID on every signup and never restores a deleted one, so there's no "restore my account" path, by design as much as by platform constraint. There is no other route to a user's own hard deletion in this schema; every other flow here (games, groups) soft-deletes.
...
- **Web analytics** — web-only (Vercel Web Analytics has no mobile equivalent), via `@vercel/analytics`. `<Analytics />` in `apps/web/src/app/layout.tsx` tracks page views automatically. Custom events live in `apps/web/src/lib/analytics.ts`: an `AnalyticsEvent` name map plus a `trackEvent()` wrapper around `@vercel/analytics/server`'s `track()`, called from Server Actions at the point a mutation succeeds (before any `redirect()`, since `redirect()` unwinds via a thrown exception and skips everything after it) — covers signup, game creation, game signup (with a `waitlisted` property), game withdrawal, group creation, friend requests sent/accepted, game results, ratings, and bug reports. Like Sentry, both page views and custom events only report from deployed builds; local dev just logs to the console instead (`@vercel/analytics/server`'s own `NODE_ENV` check) and doesn't consume quota. Add new events to the `AnalyticsEvent` map rather than passing raw strings to `track()` — see README's "Web Analytics (Vercel)" section for the Hobby-tier event cap and dashboard link.
...
- **Games** — create a game (friends-invite mode or group mode), set kickoff time / "visible from" time / team names / players-per-team; players sign up and get waitlisted once capacity (`players_per_team * 2`) is exceeded; withdraw; admins can add/remove guest "ringers", invite more friends, and notify players of team assignments. The creator (specifically — not any group admin) can soft-delete a game (`deleted_at`, via the `delete_game` RPC); once deleted it disappears from every read, for everyone, with no recovery UI. On the games list screen, a game not yet visible to everyone (before `visible_at`) still shows to its admin/creator as an early preview, greyed out and non-clickable (`getGameVisibilityStatus` in `packages/shared`) — distinct from being fully open, which only happens once `visible_at` passes. Group-scoped game listings apply the same gating (web's `/groups/[id]/games` and the group detail page's "Upcoming Games" card; mobile's `GroupGamesScreen`/`GroupDetailScreen` via `useGroupUpcomingGames`) — this used to be a real gap (any group member could see and sign up for a not-yet-visible game through these routes, since `can_view_game`'s RLS only checks group membership, never `visible_at`), fixed by reusing `getGameVisibilityStatus` there too rather than querying `games` unfiltered. The game detail page itself has no equivalent visibility gating (`can_view_game`'s RLS check doesn't consider `visible_at` at all — only group membership, creator, or an explicit invite), so web's post-creation redirect checks `visible_at` before deciding where to land: straight into the new game if it's already visible, back to the games list otherwise, rather than walking the creator into a page they can't yet reach through the app's own UI. Mobile's create-game flow always returns to the list regardless, which sidesteps the question entirely.
...
- **Notifications** — push notifications (mobile only, Expo push, requires a physical device) for friend requests, group/game invites, team assignments, ringers-opened, and new-game-visible (via a `pg_cron`-triggered edge function sweep); in-app badge counts on Friends/Groups tabs via realtime subscriptions (both apps); email for the same events (via Resend, `supabase/functions/_shared/email.ts`) as web's out-of-band channel since it has no push registration — sent independently of push, so mobile users get both and web-only users still get notified. Delivery is gated per (user, notification type, channel) by the `notification_preferences` table (`user_id`, `notification_type`, `push_enabled`, `email_enabled`) rather than the old single global `profiles.notifications_enabled` switch it replaced — rows are sparse overrides (a type a user has never touched has no row and defaults to both channels enabled), merged onto defaults via `mergeNotificationPreferences` in `packages/shared` (also home to the canonical `NOTIFICATION_TYPES`/`NOTIFICATION_TYPE_LABELS`, since both apps' preference UIs and the merge logic need the same list — the two edge functions still hand-duplicate their own copy of the type union, matching this codebase's existing pattern of not importing `packages/shared` from Deno). Both apps expose this as a matrix (notification type × push/email, with a "select all" per channel) — mobile via Profile → "Notifications" (`NotificationPreferencesScreen`), web via `/profile/notifications` — each toggle upserting immediately rather than requiring a save button. `send-notification` and `sweep-visible-games` both look up the recipient's preference row for the relevant type before sending each channel (no row ⇒ enabled), independently of each other, same as the push/email independence they already had.
...
- `lib/analytics.ts` — Vercel Web Analytics custom events (see "Web analytics" under Features above). Server Actions import `trackEvent`/`AnalyticsEvent` from here instead of calling `@vercel/analytics/server`'s `track()` directly, so event names stay centralized and typo-checked.
...
- `migrations/` — applied in filename (timestamp) order. Current schema is split across:
  ...
  - Later migrations add `groups`/`group_members`/`group_invitations`/`game_invitations` (group + invite scoping of game visibility), notification sweep infra (`visibility_notified_at` + `pg_cron`), ringers (`saved_ringers`, `is_ringer`/`guest_name` on `player_games`), game results (`result_team1_score`/`result_team2_score`/`result_outcome` + `set_game_result` RPC), game soft-delete (`games.deleted_at` + `delete_game` RPC, creator-only — deliberately narrower than `is_game_admin`, which also covers any group admin; `can_view_game` excludes soft-deleted rows for everyone), `avatars` bucket size/mime restrictions (`file_size_limit`, `allowed_mime_types`, backstopping the client-side `validateAvatarFile` check), `bug_reports` (RLS: own-rows SELECT + INSERT, though the actual insert path goes through the `send-bug-report` edge function's service-role client), group soft-delete (`groups.deleted_at` + `delete_group` RPC, any group admin — matches `is_group_admin` exactly, unlike `delete_game`'s narrower creator-only check — which also soft-deletes every game scoped to that group; the deleted check is baked into `is_group_member`/`is_group_admin` themselves rather than only the `groups` table's SELECT policy), and a fix for `groups.created_by`'s FK to `ON DELETE SET NULL` (it was `ON DELETE CASCADE` — a pre-existing bug independent of account deletion, since it meant deleting *any* group creator, e.g. via the Supabase dashboard, hard-deleted the whole group and every game in it for every other member; now matches `games.created_by`, added to support account deletion below).
