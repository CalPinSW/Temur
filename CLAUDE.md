...
- **Notifications** — push notifications (mobile only, Expo push, requires a physical device) for friend requests, group/game invites, team assignments, ringers-opened, and new-game-visible (via a `pg_cron`-triggered edge function sweep); in-app badge counts on Friends/Groups tabs via realtime subscriptions.
