...
Two pieces are already wired up in this repo; two env vars remain to be set from your Apple/Google developer accounts before it actually verifies:

- **`apps/mobile/app.json`** declares `ios.associatedDomains: ["applinks:www.temur.app"]` and an `autoVerify` Android `https` intent filter for `www.temur.app` — this is what tells each OS to trust that domain and (for iOS) triggers the device to fetch and cache the AASA file below on install/update. Neither the Team ID nor the Android fingerprint needs to live here — Xcode/EAS resolve the signing team from your build credentials automatically, and the fingerprint is only ever *read out of* the Android release keystore, never fed into a mobile build as a config value.
- **`apps/web/src/app/.well-known/apple-app-site-association/route.ts`** and **`apps/web/src/app/.well-known/assetlinks.json/route.ts`** are what each OS fetches from `https://www.temur.app/.well-known/...` to verify the app is actually authorized for that domain (the mechanism that stops any other app from claiming your links). Both read their values from `APPLE_TEAM_ID`/`ANDROID_SHA256_CERT_FINGERPRINT` env vars (server-only — see `apps/web/.env.local.example`) rather than hardcoding them, so rotating a signing key or moving to a new Apple team doesn't require a code change. Only the paths notification emails actually link to (`/games`, `/games/*`, `/friends`, `/friends/requests`, `/groups/invites`, `/reset-password`) are claimed — everything else (login, profile, group detail, etc.) has no matching mobile screen yet and keeps opening in the browser. `apps/mobile/src/utils/deepLinkRouting.ts` is the in-app side of that same route table — it maps an incoming `temur://` or `https://www.temur.app/...` link to the matching screen (`useAuthDeepLinks` wires it in); update both together if you add a new claimed path.

Setup steps:

1. **iOS** — find your Apple Developer **Team ID** (developer.apple.com → Membership, or `eas credentials`), then set it as `APPLE_TEAM_ID` in `apps/web/.env.local` locally and as a Vercel project env var in production (it's used as `TEAMID.com.calpin.temur`). Deploy web, then verify with `curl -s https://www.temur.app/.well-known/apple-app-site-association` (should return valid JSON) and Apple's own validator (https://search.developer.apple.com/appsearch-validation-tool/). iOS caches AASA lookups at install/update time, so a fresh install may be needed to see a change take effect. The same Team ID also drives `eas submit` for App Store submission — see `apps/mobile/BUILD.md`, which reads it from an `APPLE_TEAM_ID` EAS environment variable rather than a second copy.
2. **Android** — once you have a release signing key (EAS-managed or your own keystore), get its SHA-256 fingerprint via `eas credentials` (Android → your build profile → Keystore) or `keytool -list -v -keystore <path> -alias <alias>`, then set it as `ANDROID_SHA256_CERT_FINGERPRINT` in `apps/web/.env.local` locally and as a Vercel project env var in production (colon-separated hex, as printed). Deploy web, then verify with `adb shell pm get-app-links com.calpin.temur` after installing a release build, or Google's Statement List Generator/Tester (https://developers.google.com/digital-asset-links/tools/generator).
